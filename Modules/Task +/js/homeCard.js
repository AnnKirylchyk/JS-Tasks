...
function renderTable() {
  ...
  products.forEach(product => {
    ...
    const editBtn = components.getButtonEl("Редактировать")   // добавляем кнопку редактирования строки
    btnWrap.append(editBtn)
    row.lastElementChild.append(btnWrap)

    editBtn.addEventListener("click", function () {
      localStorageFunctions.setItemLc ("editProduct", product)  // передаем данные редактируемого элементы в local storage
      navigate("edit")
    })
